...
import TiltCard from "../Animateds/TiltCards";

export default function AboutCard({ Icon, description, gradient, title, idx = 0 }: IAboutCardProps): ReactNode {
	return (
		<TiltCard key={title} className="group flex-1 hover:cursor-pointer">
			<m.div
				key={title}
				viewport={{ once: true }}
				...
				<Card className="bg-card/50 backdrop-blur-sm border-border/50 hover:border-primary/50 transition-all duration-300 h-full w-full overflow-hidden">
